...
locally_level_out_pmmh = biips_pmmh_samples(obj_pmmh, n_iter, n_part, thin=thin) # samples
...
p<- p+ geom_line(data = data.frame(x=seq(1,length(locally_level_model_summary$x$mean)),y=exp(locally_level_model_summary$x$mean)), aes(x = x, y = y), color = "red") +
 ...
 geom_ribbon(data=data.frame(x=seq(1,length(ma_data)),y=ma_data),aes(x=x,ymin=locally_level_model_summary$x$quant$`0.025`,ymax=locally_level_model_summary$x$quant$`0.975`),alpha=0.3)+
  xlab('data_date') +
  ylab('count')
print(p)
...
qqnorm(ma_data-locally_level_model_summary$x$mean)
qqline(ma_data-locally_level_model_summary$x$mean)
...
$$E(Z_{t+1} | Z_t) = z_t$$
$$E(Y_t |Z_t) = exp(z_t)$$
so predictions for all $h$ steps ahead will remain at a constant value, $exp(z_t)$. 


Although the qq-plot is based off a very small sample size, the residuals do seem to be normally distributed. 
...
p<- p+ geom_line(data = data.frame(x=seq(1,length(seasonal_model_summary$x$f$mean[1,])),y=exp(seasonal_model_summary$x$f$mean[1,])), aes(x = x, y = y), color = "red") +
  ...
  geom_line(data=data.frame(x=seq(1,length(ma_data)),y=ma_data), aes(x = x, y = y), color = "cornflowerblue") +
 #geom_ribbon(data=data.frame(x=seq(1,length(ma_data)),y=exp(summ_pmmh$x$f$mean[1,])),aes(x=x,ymin=exp(summ_pmmh$x$f$quant$`0.025`[1,]),ymax=(summ_pmmh$x$f$quant$`0.975`[1,])),alpha=0.3)+
  xlab('data_date') +
  ylab('count')
print(p)

qqnorm(ma_data-seasonal_model_summary$x$f$mean[1,])
qqline(ma_data-seasonal_model_summary$x$f$mean[1,])
...
### Model Comparison
In order to evaluate model fit we can compare the difference in the log-likelihoods of each model. We cannot, however, perform any hypothesis testing because the degrees of freedom are undefined (since every $Y_t$ is assigned a $Z_t$, the df is technically $0$ for all models).
$$D = -2(ln(L(D | M_1)) - ln(L(D |M_2))$$

```{r,echo=FALSE,message=FALSE }

D_locally_level_vs_seasonal <- -2*(as.numeric(seasonal_out_smc$log_marg_like)-mean(as.numeric(locally_level_out_pmmh$log_marg_like_pen)))

print (D_locally_level_vs_seasonal)
```
...
#setting the mean value of the initial count to 1400
ar_data = list(t_max=t_max, y = ma_data,   mean_sigma_init = c(log(1400)), cov_sigma_init=diag(2) ,mean_x_init=c(log(1400),log(1400)))
ar_model = biips_model(ar_model_file, data=ar_data,sample_data = FALSE)

##fixing variance for now, will extend model to handle inference over variance later

n_part = 100000 # Number of particles
variables = c('x') # Variables to be monitored
ar_out_smc = biips_smc_samples(ar_model, variables, n_part)

ar_summ_pmmh = biips_summary(ar_out_smc, probs=c(.025, .975))



p<- ggplot() 
p<- p+ geom_line(data = data.frame(x=seq(1,length(ar_summ_pmmh$x$f$mean)),y=exp(ar_summ_pmmh$x$f$mean)), aes(x = x, y = y), color = "red") +
  
  geom_line(data=data.frame(x=seq(1,length(ma_data)),y=ma_data), aes(x = x, y = y), color = "cornflowerblue") +
 geom_ribbon(data=data.frame(x=seq(1,length(ma_data)),y=ma_data),aes(x=x,ymin=ar_summ_pmmh$x$f$quant$`0.025`,ymax=ar_summ_pmmh$x$f$quant$`0.975`),alpha=0.3)+
  xlab('data_date') +
  ylab('count')
print(p)
...
Although we cannot do strict hypothesis testing using the deviance, we can still compare deviances to get a sense of model preformance. 

```{r,echo=FALSE,message=FALSE}
D_locally_level_vs_seasonal <- -2*(as.numeric(seasonal_out_smc$log_marg_like)-mean(as.numeric(locally_level_out_pmmh$log_marg_like_pen)))


D_locally_level_vs_ar <- -2*(as.numeric(ar_out_smc$log_marg_like)-mean(as.numeric(locally_level_out_pmmh$log_marg_like_pen)))

print (D_locally_level_vs_seasonal)
print(D_locally_level_vs_ar)
```

We can see that the $AR(2)$ model seems to fit the model better, however, we cannot conclude this in a hypothesis testing framework.
